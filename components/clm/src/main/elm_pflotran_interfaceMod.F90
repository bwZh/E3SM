...
#include "petsc/finclude/petscviewer.h"
  ...
  public :: elm_pf_interface_init     , &
       elm_pf_set_sflow_forcing       , &
       elm_pf_update_soil_moisture    , &
       elm_pf_update_soil_temperature , &
       elm_pf_update_soil_therm_cond  , &
       elm_pf_update_drainage         , &
       elm_pf_update_h2osfc           , &
       elm_pf_step_th                 , &
       elm_pf_write_restart           , &
       elm_pf_set_restart_stamp       , &
       elm_pf_vecget_gflux            , &
       elm_pf_vecrestore_gflux
  ...
  private :: interface_init_clm_pf    , & ! Phase one initialization
       ...
       vecget_gflux_subsurf_clm_pf    , &
       vecrestore_gflux_subsurf_clm_pf
#endif
...
contains
  ...
  !-----------------------------------------------------------------------------
  subroutine elm_pf_interface_init(bounds, col, lun, grc, &
    ...
    type(gridcell_physical_properties_type) , intent(in)    :: grc
    type(column_physical_properties)        , intent(in)    :: col
    type(landunit_physical_properties)      , intent(in)    :: lun
  ...
  !-----------------------------------------------------------------------------
  subroutine elm_pf_set_sflow_forcing(num_hydrologyc, filter_hydrologyc, &
    ...
    call set_sflow_forcing_clm_pf(num_hydrologyc, filter_hydrologyc, &
         atm2lnd_vars, waterflux_vars)
#else
    character(len=256) :: subname = "elm_pf_set_sflow_forcing"
    call pflotran_not_available(subname)
#endif

  end subroutine elm_pf_set_sflow_forcing


  !-----------------------------------------------------------------------------
  subroutine elm_pf_update_soil_moisture(waterstate_vars, soilstate_vars, bounds, &
...
#ifdef ELM_PFLOTRAN
    call update_soil_moisture_clm_pf(waterstate_vars, soilstate_vars, &
         bounds, num_hydrologyc, filter_hydrologyc, col, soilhydrology_vars)
#else
    character(len=256) :: subname = "elm_pf_update_soil_moisture"
    call pflotran_not_available(subname)
#endif
  end subroutine elm_pf_update_soil_moisture


  !-----------------------------------------------------------------------------
  subroutine elm_pf_update_soil_temperature(bounds, &
    ...
    call update_soil_temperature_clm_pf(bounds, &
         num_nolakec, filter_nolakec, temperature_vars, col)
#else
    character(len=256) :: subname = "elm_pf_update_soil_temperature"
    call pflotran_not_available(subname)
#endif
  end subroutine elm_pf_update_soil_temperature

  !-----------------------------------------------------------------------------
  subroutine elm_pf_update_soil_therm_cond(bounds, &
    ...
    character(len=256)              :: subname = "elm_pf_update_soil_therm_cond"
    ...
    call pflotran_not_available(subname)
#endif
  end subroutine elm_pf_update_soil_therm_cond


  !-----------------------------------------------------------------------------
  subroutine elm_pf_update_drainage(num_hydrologyc, filter_hydrologyc, waterflux_vars)
...
#else
    character(len=256) :: subname = "elm_pf_update_drainage"
    call pflotran_not_available(subname)
#endif

  end subroutine elm_pf_update_drainage

  !-----------------------------------------------------------------------------
  subroutine elm_pf_update_h2osfc(num_hydrologyc, filter_hydrologyc, &
       waterstate_vars)
...
#else
    character(len=256) :: subname = "elm_pf_update_h2osfc"
    call pflotran_not_available(subname)
#endif

  end subroutine elm_pf_update_h2osfc


  !-----------------------------------------------------------------------------
  subroutine elm_pf_step_th(bounds, &
    ...
    type(soilstate_type)                 , intent(inout) :: soilstate_vars
    type(vegetation_physical_properties) , intent(in)    :: patch_vars
    ...
    call step_th_clm_pf(bounds, &
         ...
         waterflux_vars, waterstate_vars, soilstate_vars, patch_vars)
#else
    character(len=256) :: subname = "elm_pf_step_th"
    call pflotran_not_available(subname)
#endif
  end subroutine elm_pf_step_th


  !-----------------------------------------------------------------------------
  subroutine elm_pf_vecget_gflux(gflux_clm_loc)
...
#else
    character(len=256) :: subname = "elm_pf_vecget_gflux()"
    call pflotran_not_available(subname)
#endif
  end subroutine elm_pf_vecget_gflux



  !-----------------------------------------------------------------------------
  subroutine elm_pf_vecrestore_gflux(gflux_clm_loc)
...
#else
    character(len=256) :: subname = "elm_pf_vecrestore_gflux()"
    call pflotran_not_available(subname)
#endif
  end subroutine elm_pf_vecrestore_gflux
  ...
  subroutine interface_init_clm_pf(bounds, col, lun, grc, &
    ...
    use abortutils        , only : endrun
    use clm_varctl        , only : use_clm_soils
    ...
    integer  :: local_num_t       ! local number of topounits across this processor
    ...
    character(len= 32) :: subname = 'elm_pf_interface_init' ! subroutine name
    ...
    associate( &
      ...
      call get_proc_total(iam, local_num_g, local_num_t, local_num_l, local_num_c, local_num_p, local_num_cohorts)
      ...
      if (use_clm_soils) then
         call SetCLMSoilPropertiesToPFLOTRAN(bounds, col, lun, grc, &
              soilstate_vars, waterstate_vars, temperature_vars, &
              soilhydrology_vars)
      else
         call GetCLMSoilPropertiesFromPFLOTRAN(bounds, col, lun, grc, &
              soilstate_vars, waterstate_vars, temperature_vars, &
              soilhydrology_vars)
      endif
  ...
  !-----------------------------------------------------------------------
  subroutine GetCLMSoilPropertiesFromPFLOTRAN(bounds, col, lun, grc, &
       soilstate_vars, waterstate_vars, temperature_vars, &
       soilhydrology_vars)
    !
    ! !DESCRIPTION:
    ...
    !
    character(len=256) :: locfn                    ! local filEname
    character(len= 32) :: subname = 'GetCLMSoilPropertiesFromPFLOTRAN' ! subroutine name
    integer :: nlevmapped

    !PetscViewer :: viewer
    PetscScalar, pointer :: hksat_z2_clm_loc(:) ! hydraulic conductivity in z-dir at saturation (mm H2O /s)
    PetscScalar, pointer :: watsat2_clm_loc(:)  ! minimum soil suction (mm)
    PetscScalar, pointer :: sucsat2_clm_loc(:)  ! volumetric soil water at saturation (porosity)
    PetscScalar, pointer :: bsw2_clm_loc(:)     ! Clapp and Hornberger "b"
    PetscErrorCode :: ierr
    ...
    associate( &
      ...
      call VecGetArrayF90(clm_pf_idata%hksat_z2_clm, hksat_z2_clm_loc, ierr)
      call VecGetArrayF90(clm_pf_idata%sucsat2_clm,  sucsat2_clm_loc,  ierr)
      call VecGetArrayF90(clm_pf_idata%watsat2_clm,  watsat2_clm_loc,  ierr)
      call VecGetArrayF90(clm_pf_idata%bsw2_clm,     bsw2_clm_loc,     ierr)
      ...
      ! Get PFLOTRAN's soil properties
      call pflotranModelGetSoilProp(pflotran_m)
      ...
      call VecRestoreArrayF90(clm_pf_idata%hksat_z2_clm, hksat_z2_clm_loc, ierr)
      call VecRestoreArrayF90(clm_pf_idata%sucsat2_clm,  sucsat2_clm_loc,  ierr)
      call VecRestoreArrayF90(clm_pf_idata%watsat2_clm,  watsat2_clm_loc,  ierr)
      call VecRestoreArrayF90(clm_pf_idata%bsw2_clm,     bsw2_clm_loc,     ierr)

    end associate

  end subroutine GetCLMSoilPropertiesFromPFLOTRAN
  ...
  subroutine set_et_forcing(bounds, &
    ...
    associate( &
         ...
         pwtgcell            => veg_pp%wtgcell                           , & !  [real(r8) (:)]  weight relative to gridcell for each pft
         pwtcol              => veg_pp%wtcol                             , & !  [real(r8) (:)]  weight relative to column for each pft
         
         pfti                => col_pp%pfti                              , & !  [integer (:)]  beginning pft index for each column
      ...
      do c = bounds%begc, bounds%endc
         if (col_pp%active(c)) then
            ...
            qflx_clm_loc(gcount*nlevmapped + j) = qflx_clm_loc(gcount*nlevmapped + j) + &
                 waterflux_vars%qflx_infl_col(c)*col_pp%wtgcell(c)*area
         end if
      enddo
  ...
  subroutine update_soil_therm_cond_clm_pf(bounds, &
    ...
    associate( &
         cgridcell  =>  col_pp%gridcell    & !  [integer (:)]  column's gridcell
         )
...
#endif
end module elm_pflotran_interfaceMod
